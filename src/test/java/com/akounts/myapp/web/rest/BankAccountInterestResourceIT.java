...
class BankAccountInterestResourceIT {
    ...
    void createBankAccountInterest() throws Exception {
        ...
        BankAccountInterestDTO bankAccountInterestDTO = bankAccountInterestMapper.toDto(bankAccountInterest);
        restBankAccountInterestMockMvc
            .perform(
                post(ENTITY_API_URL)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(bankAccountInterestDTO))
            )
    ...
    void putNewBankAccountInterest() throws Exception {
        ...
        updatedBankAccountInterest
            ...
            .scrappingTag(UPDATED_SCRAPPING_TAG)
            .scrappingTagBis(UPDATED_SCRAPPING_TAG_BIS);
        BankAccountInterestDTO bankAccountInterestDTO = bankAccountInterestMapper.toDto(updatedBankAccountInterest);

        restBankAccountInterestMockMvc
            .perform(
                put(ENTITY_API_URL_ID, bankAccountInterestDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(bankAccountInterestDTO))
            )
            .andExpect(status().isOk());
    ...
    void patchNonExistingBankAccountInterest() throws Exception {
        ...
        restBankAccountInterestMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, bankAccountInterestDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(bankAccountInterestDTO))
            )
            .andExpect(status().isBadRequest());
