...
class BankProjectResourceIT {
    ...
    void createBankProject() throws Exception {
        ...
        BankProjectDTO bankProjectDTO = bankProjectMapper.toDto(bankProject);
        restBankProjectMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bankProjectDTO))
            )
    ...
    void putNewBankProject() throws Exception {
        ...
        updatedBankProject
            ...
            .initialValue(UPDATED_INITIAL_VALUE)
            .currentValue(UPDATED_CURRENT_VALUE);
        BankProjectDTO bankProjectDTO = bankProjectMapper.toDto(updatedBankProject);

        restBankProjectMockMvc
            .perform(
                put(ENTITY_API_URL_ID, bankProjectDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(bankProjectDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamBankProject() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBankProjectMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bankProjectDTO)))
    ...
    void patchNonExistingBankProject() throws Exception {
        ...
        restBankProjectMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, bankProjectDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(bankProjectDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamBankProject() throws Exception {
        ...
        restBankProjectMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(bankProjectDTO))
            )
            .andExpect(status().isMethodNotAllowed());
