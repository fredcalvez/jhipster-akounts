...
class BankStreamTransactionResourceIT {
        ...
        restBankStreamTransactionMockMvc
            .perform(
                post(ENTITY_API_URL)
                    ...
                    .content(TestUtil.convertObjectToJsonBytes(bankStreamTransaction))
            )
    ...
    void putNewBankStreamTransaction() throws Exception {
        ...
        restBankStreamTransactionMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedBankStreamTransaction.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(updatedBankStreamTransaction))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingBankStreamTransaction() throws Exception {
        ...
        restBankStreamTransactionMockMvc
            .perform(
                put(ENTITY_API_URL_ID, bankStreamTransaction.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(bankStreamTransaction))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchNonExistingBankStreamTransaction() throws Exception {
        ...
        restBankStreamTransactionMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, bankStreamTransaction.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(bankStreamTransaction))
            )
            .andExpect(status().isBadRequest());
