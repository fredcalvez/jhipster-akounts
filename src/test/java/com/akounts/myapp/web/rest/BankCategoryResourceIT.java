...
class BankCategoryResourceIT {
    ...
    void createBankCategory() throws Exception {
        ...
        BankCategoryDTO bankCategoryDTO = bankCategoryMapper.toDto(bankCategory);
        restBankCategoryMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bankCategoryDTO))
            )
    ...
    void putNewBankCategory() throws Exception {
        ...
        updatedBankCategory
            ...
            .income(UPDATED_INCOME)
            .isexpense(UPDATED_ISEXPENSE);
        BankCategoryDTO bankCategoryDTO = bankCategoryMapper.toDto(updatedBankCategory);

        restBankCategoryMockMvc
            .perform(
                put(ENTITY_API_URL_ID, bankCategoryDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(bankCategoryDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamBankCategory() throws Exception {
        ...
        restBankCategoryMockMvc
            .perform(
                put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bankCategoryDTO))
            )
    ...
    void patchNonExistingBankCategory() throws Exception {
        ...
        restBankCategoryMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, bankCategoryDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(bankCategoryDTO))
            )
            .andExpect(status().isBadRequest());
