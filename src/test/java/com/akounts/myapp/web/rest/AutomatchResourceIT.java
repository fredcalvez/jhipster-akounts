...
class AutomatchResourceIT {
    ...
    void createAutomatch() throws Exception {
        ...
        AutomatchDTO automatchDTO = automatchMapper.toDto(automatch);
        restAutomatchMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(automatchDTO)))
    ...
    void putNewAutomatch() throws Exception {
        ...
        updatedAutomatch
            ...
            .useCount(UPDATED_USE_COUNT)
            .defaultTag(UPDATED_DEFAULT_TAG);
        AutomatchDTO automatchDTO = automatchMapper.toDto(updatedAutomatch);

        restAutomatchMockMvc
            .perform(
                put(ENTITY_API_URL_ID, automatchDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(automatchDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAutomatch() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAutomatchMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(automatchDTO)))
    ...
    void patchNonExistingAutomatch() throws Exception {
        ...
        restAutomatchMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, automatchDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(automatchDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAutomatch() throws Exception {
        ...
        restAutomatchMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(automatchDTO))
            )
            .andExpect(status().isMethodNotAllowed());
