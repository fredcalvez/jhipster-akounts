...
class RebaseHistoryResourceIT {
    ...
    private static final Currency DEFAULT_OLD_CURRENCY = Currency.AED;
    private static final Currency UPDATED_OLD_CURRENCY = Currency.AFN;
    ...
    private static final Currency DEFAULT_NEW_CURRENCY = Currency.AED;
    private static final Currency UPDATED_NEW_CURRENCY = Currency.AFN;
    ...
    void createRebaseHistory() throws Exception {
        ...
        RebaseHistoryDTO rebaseHistoryDTO = rebaseHistoryMapper.toDto(rebaseHistory);
        restRebaseHistoryMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(rebaseHistoryDTO))
            )
    ...
    void putNewRebaseHistory() throws Exception {
        ...
        updatedRebaseHistory
            ...
            .newCurrency(UPDATED_NEW_CURRENCY)
            .runDate(UPDATED_RUN_DATE);
        RebaseHistoryDTO rebaseHistoryDTO = rebaseHistoryMapper.toDto(updatedRebaseHistory);

        restRebaseHistoryMockMvc
            .perform(
                put(ENTITY_API_URL_ID, rebaseHistoryDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(rebaseHistoryDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamRebaseHistory() throws Exception {
        ...
        restRebaseHistoryMockMvc
            .perform(
                put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(rebaseHistoryDTO))
            )
    ...
    void patchNonExistingRebaseHistory() throws Exception {
        ...
        restRebaseHistoryMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, rebaseHistoryDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(rebaseHistoryDTO))
            )
            .andExpect(status().isBadRequest());
