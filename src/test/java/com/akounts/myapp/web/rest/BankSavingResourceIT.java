...
class BankSavingResourceIT {
    ...
    void createBankSaving() throws Exception {
        ...
        BankSavingDTO bankSavingDTO = bankSavingMapper.toDto(bankSaving);
        restBankSavingMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bankSavingDTO)))
    ...
    void putNewBankSaving() throws Exception {
        ...
        BankSavingDTO bankSavingDTO = bankSavingMapper.toDto(updatedBankSaving);

        restBankSavingMockMvc
            .perform(
                put(ENTITY_API_URL_ID, bankSavingDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(bankSavingDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamBankSaving() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBankSavingMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bankSavingDTO)))
    ...
    void patchNonExistingBankSaving() throws Exception {
        ...
        restBankSavingMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, bankSavingDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(bankSavingDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamBankSaving() throws Exception {
        ...
        restBankSavingMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(bankSavingDTO))
            )
            .andExpect(status().isMethodNotAllowed());
